// https://github.com/kangax/compat-table/blob/gh-pages/master.js
...
$(function() {
  ...
  window.__updateSupertest = function() {
    ...
    tr
      ...
      .each(function () {
        var td = $(this)
        var browser = td.data('browser')
        var tally = subtests.find('.yes[data-browser=' + browser + ']').length;
        var grade = tally/subtests.length;
        td.attr('data-tally', grade)
        td.css({
          'background-color': 'hsl(' + (120*grade|0) + ',' +((86 - (grade*44))|0)  +'%,50%)'
        })
        td.text(tally + '/' + subtests.length)
      })
  };
  ...
  window.onhashchange = function() {
    if (location.hash) {
      ...
      // (The CSS widens the selected column only horizontally.)
        ...
        var td = table.find('td[data-browser="' + elem.attr('href').slice(1) + '"]')
  ...
  window.__updateHeaderTotal = function(i) {
    ...
    console.log(yesPoints, totalPoints)
  ...
  var doSort = function (sortVal) {
    var sortSpecMap = {
      'features':         {attr: 'data-features', order: 1, hidePlatformtype: false},
    ...
    var sortSpec = sortSpecMap[sortVal];
    ...
    table.detach().find('tr').each(function(i, row) {
      ...
      var cells = $(row.cells).slice(0).toArray().sort(comparator);
